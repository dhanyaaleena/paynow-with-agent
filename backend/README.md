...
### GET /api/decide - Get Recent Decisions
```sh
curl -H "X-API-Key: test-api-key" http://localhost:8000/api/decide
```
...
- **Implementation:** See `rate_limiter.py`. Used as a FastAPI dependency in the `/api/decide` endpoint.
